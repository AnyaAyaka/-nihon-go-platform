...
export default function StudentDashboard({ user, profile }) {
  ...
  const handleRescheduleLesson = () => {
    ...
    if (!canReschedule) { alert('Cannot reschedule within 24 hours of the lesson.'); return }
  ...
  const handleContactUs = () => { window.location.href = 'mailto:info@nihongolondon.com?subject=Support Request from Nihon GO! Platform' }
  ...
  const getLevelUrl = (level) => {
    ...
    return urls[level] || 'https://nihongo-world.com/materials/jlpt/'
  }

  return (
    <div style={{ padding: '40px 24px', maxWidth: '1400px', margin: '0 auto', background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)', minHeight: '100vh' }}>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '30px', maxWidth: '500px', width: '90%', maxHeight: '80vh', overflow: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <button onClick={() => { setShowLessonDialog(false); setSelectedLesson(null) }} disabled={isCancelling} style={{ padding: '12px', borderRadius: '8px', border: '2px solid #e1e5e9', background: 'white', cursor: isCancelling ? 'not-allowed' : 'pointer', fontSize: '16px', fontWeight: '600' }}>Close</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              ...
              {tierInfo.tier !== 'none' && <div style={{ fontSize: '16px', opacity: 0.9, marginTop: '4px' }}>{tierConfig.discount} OFF all purchases</div>}
            </div>
          </div>
      ...
      {/* Upcoming Lessons */}
      <div style={{ background: 'white', borderRadius: '16px', padding: '24px 30px', marginBottom: '40px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', border: '1px solid #e2e8f0' }}>
        ...
        {loading ? <p style={{ margin: 0, color: '#64748b', fontSize: '15px' }}>Loading...</p>
          : upcomingLessons.length > 0 ? (
            <div style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '8px' }}>
              {upcomingLessons.map((lesson) => (
                <div key={lesson.id} onClick={() => { setSelectedLesson(lesson); setShowLessonDialog(true) }}
                  style={{ minWidth: '280px', padding: '20px', background: 'linear-gradient(135deg, #f8fafc 0%, #e0f2fe 100%)', borderRadius: '12px', border: '2px solid #e0f2fe', transition: 'all 0.2s ease', cursor: 'pointer' }}
      ...
      {/* JLPT Section - shown above main cards if active */}
      {jlptSubscriptions.length > 0 && (
        <div style={{ background: 'linear-gradient(135deg, #1e293b 0%, #334155 100%)', color: 'white', borderRadius: '16px', padding: '24px 30px', marginBottom: '24px', boxShadow: '0 8px 24px rgba(0,0,0,0.2)' }}>
          <h3 style={{ margin: '0 0 16px 0', fontSize: '20px', fontWeight: '700' }}>JLPT Mock Tests</h3>
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
            {jlptSubscriptions.map((sub) => (
              <a key={sub.level} href={getLevelUrl(sub.level)}
                style={{ background: 'rgba(255,255,255,0.1)', borderRadius: '10px', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: '4px', textDecoration: 'none', border: '1px solid rgba(255,255,255,0.15)', transition: 'background 0.2s' }}
                onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.18)'}
                onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'}
              >
                <span style={{ fontWeight: '700', fontSize: '20px', color: 'white' }}>{sub.level}</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '30px' }}>
        ...
        >
          ...
          {loading ? <p style={{ margin: '0 0 24px 0', fontSize: '16px', opacity: '0.9', textAlign: 'center' }}>Loading...</p>
            : allTickets.length > 0 && totalTickets > 0 ? (
              <div style={{ margin: '0 0 24px 0', textAlign: 'center' }}>
                ...
                <div style={{ marginTop: '16px', fontSize: '13px', opacity: '0.85', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  {allTickets.map(ticket => <div key={ticket.ticket_type}>{ticket.ticket_type.replace('_', ' ')}: {ticket.remaining_tickets}</div>)}
                </div>
              </div>
            ) : <p style={{ margin: '0 0 24px 0', fontSize: '16px', opacity: '0.9', textAlign: 'center' }}>No tickets available</p>
          }
          <button onClick={() => router.push('/subscription')}
            style={{ background: 'rgba(255,255,255,0.25)', color: 'white', border: '2px solid white', borderRadius: '12px', padding: '14px 28px', fontSize: '16px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s ease', width: '100%' }}
        ...
        >
          ...
          <button onClick={() => router.push('/booking')}
            style={{ background: '#1e293b', color: 'white', border: 'none', borderRadius: '12px', padding: '14px 28px', fontSize: '16px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s ease', width: '100%' }}
        ...
        >
          ...
          <p style={{ margin: '0 0 28px 0', color: '#64748b', fontSize: '15px', lineHeight: '1.6', textAlign: 'center' }}>Need help? Our support team is here to assist you</p>
